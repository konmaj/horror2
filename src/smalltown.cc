...
SmallTown::Status::Status(const std::string &monsterName, HealthPoints monsterHealth, size_t aliveCount) : monsterName_(
        monsterName), monsterHealth_(monsterHealth), aliveCount_(aliveCount) {}
...
SmallTown::Status SmallTown::getStatus() const {
    return Status(monster_->getName(), monster_->getHealth(), countAliveCitizens());
}
...
void SmallTown::checkState() const {
    ...
    bool citizensAlive = countAliveCitizens() > 0;
...
size_t SmallTown::countAliveCitizens() const {
    size_t counter = 0;
    for (auto &citizen : citizens_) {
...
SmallTown::SmallTown(const std::shared_ptr<MonsterComponent> &monster,
                     const std::vector<std::shared_ptr<Citizen>> &citizens, Time startTime, Time maxTime,
                     const std::shared_ptr<AttackTime> &attackTime) : monster_(monster), citizens_(citizens),
                                                                      time_(startTime), maxTime_(maxTime),
                                                                      attackTime_(attackTime) {}
